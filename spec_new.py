...
class Spec:
    ...
    def get_dNdYPtdPt2D(self):
        ...
        weight_phi = np.pi*np.hstack((gaulew48, gaulew48[-1::-1]))        
        for i in range(NY):
            for j in range(NPT):
#              for k in range(NPHI):
                spec_along_phi = self.spec[i,j,:]
                self.dNdYPtdPt2D[i,j] = phi_integral(spec_along_phi)
#                self.dNdYPtdPt2D[i,j] += spec_along_phi*weight_phi[k]
    ######################################################################
    def get_dNdY(self):
    ...
    def get_TotN(self):
        ...
        Ntot = rapidity_integral(dNdY, ylo=-0.5, yhi=0.5)
        fout_name = self.path + "/TotN" + self.strout
        Out.append(Ntot)
        print(Ntot)
        np.savetxt(fout_name, Out, \
            header='#rapidity dN/d{rapidity}'.format( \
            rapidity=self.rapidity_kind))
    ...
    def get_dNPtdPt_over2pi(self, ylo=-1.3, yhi=1.3, comment=""):
        ...
        for j in range(NPT):
            spec_along_y = self.dNdYPtdPt2D[:,j]*PT[j]
            dNPtdPt_over2pi[j] = rapidity_integral(spec_along_y, ylo, yhi)
        
        dNPtdPt_over2pi /= ( (yhi-ylo) )
    ...
    def get_event_planes(self, ylo=-0.5, yhi=0.5):
        ...
        fout_name = self.path + "/eplane"

        np.savetxt(fout_name,np.array(self.event_plane))
